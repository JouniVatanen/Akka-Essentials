...
class TimedAssertionsSpec extends TestKit(ActorSystem("TimedAassertionsSpec", ConfigFactory.load().getConfig("specialTimedAssertionsConfig")))
  with ImplicitSender
  with WordSpecLike
  ...
  "A worker actor" should {
    ...
    "reply to a test probe in a timely manner" in {
      within(1 second) {
        val probe = TestProbe()
        probe.send(workerActor, "actor")
        probe.expectMsg(WorkResult(42)) // timeout of 0.3 seconds because of config file
      }
    }
  }
}
